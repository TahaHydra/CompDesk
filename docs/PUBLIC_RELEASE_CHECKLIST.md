...
Updated 2026-08-03: the repository-history gate below was re-evaluated after
the history-cleanup rewrite was executed, force-pushed, and independently
verified from a fresh clone of the GitHub remote.

## Beta gates (`v0.9.0-beta.1`)

| Gate | Status | Evidence |
|---|---|---|
| Repository history cleanup (formerly "repository visibility decision") | **PASS (completed and verified 2026-08-03)** | History rewritten with `git-filter-repo` after a verified mirror backup (`CompDesk-before-history-cleanup.git` + matching bundle, both preserved on disk and untouched) and pushed to all 5 human-managed branches (`main`, `release/xhydra-branding-beta`, `release/public-hardening`, `fix/settings-branding-email`, `agent/restore-latest-compdesk`). Verified from a **completely fresh `git clone` of the GitHub remote**, not just the local rewrite: (1) the forbidden historical client name/domain/development credentials (see `src/__tests__/repository-hygiene.test.ts` for the exact encoded terms it checks) — **zero matches** across every reachable commit, blob, and commit message on every branch and tag; (2) the repository author's real employer email — **zero matches** in author or committer metadata anywhere in history, rewritten on every affected commit (author and committer) to the verified GitHub noreply address `114909147+TahaHydra@users.noreply.github.com` (confirmed already in use by this exact account on an existing GitHub-authored commit, not guessed or constructed); (3) `git fsck --full` — clean, no errors; (4) the stale `v1.0.0-rc1` tag — confirmed removed from the remote (`git ls-remote --tags` returns nothing). The repository remains **private**; publishing it is a separate manual decision for the owner, no longer blocked by history content. |
...
| Gate | Status | Evidence |
|---|---|---|
| Coordinated git-history rewrite to remove the client-specific/model-vendor references and author-email exposure | **PASS (completed 2026-08-03)** | Completed via `git-filter-repo` with a verified mirror/bundle backup, mailmap-based author/committer rewrite, and fresh-clone verification against the GitHub remote — see the repository-history cleanup beta gate above for full evidence. |
...
- ~~Reconcile setup-origin naming~~ — first-run documentation now uses the implemented `SETUP_PUBLIC_ORIGIN` and `SETUP_TRUST_PROXY` variables consistently.
- ~~Add real screenshots to the README~~ — dashboard and branding settings remain in the README; non-secret wizard steps 3/5/7/10 remain in `docs/FIRST_RUN_SETUP.md`. The older authorization screenshot containing an expired real token was removed: public examples must not embed token material, even when expired.
...
1. ~~Repository visibility / history exposure~~ — **resolved 2026-08-03**. The history rewrite described in the repository-history cleanup gate above removed the pre-rebrand client name/credentials and the author's real employer email from every reachable commit, verified from a fresh clone. The repository remains private; publishing it is now a separate manual decision for the owner, not a content-risk blocker.
...
**READY FOR PUBLIC BETA WITH DOCUMENTED LIMITATIONS** for `v0.9.0-beta.1`, using the recommended bundled-PostgreSQL Docker Compose path. The repository-history cleanup is complete and independently verified; no outstanding decision blocks this gate. External PostgreSQL is a known-broken, documented limitation, not a beta blocker for the bundled-PostgreSQL path.
