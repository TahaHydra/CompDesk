...
function SmtpSettingsTab() {
    ...
    return <div className="space-y-6"><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Mail className="h-4 w-4" /> SMTP Configuration</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Username / Email</Label><Input value={smtp.smtp_user} onChange={(event) => setSmtp({ ...smtp, smtp_user: event.target.value })} /></div><div className="space-y-2"><Label>Password</Label><Input type="password" placeholder={smtpPasswordConfigured ? 'Configured; enter a replacement only' : 'Required'} value={smtp.smtp_password} onChange={(event) => setSmtp({ ...smtp, smtp_password: event.target.value })} /><p className="text-xs text-muted-foreground">Database passwords use an authenticated enc:v1 AES-256-GCM envelope. Effective source: {passwordSource === 'environment' ? 'environment (overrides the saved password)' : passwordSource === 'database' ? 'encrypted database setting' : 'not configured'}.</p></div></div>
