                ...
                <div className="space-y-2">
                    {visibleArticles.map((article) => (
                        <Card key={article.id} className="shadow-sm"><CardContent className="flex flex-col justify-between gap-4 p-4 sm:flex-row sm:items-center">
                            ...
                            <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => editArticle(article)}><Pencil className="mr-1 h-3.5 w-3.5" />{t('Edit')}</Button><Button size="sm" variant="ghost" className="text-destructive" onClick={() => window.confirm('Delete this article?') && deleteArticle.mutate(article.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div>
                        </CardContent></Card>
                    ))}
            ...
            <TabsContent value="collections" className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {collections.map((collection) => (
                        <Card key={collection.id} className="shadow-sm"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-base"><BookOpen className="h-4 w-4 text-primary" />{collection.titleEn}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{collection.titleFr}</p></div><Badge variant={collection.isPublished ? 'secondary' : 'outline'}>{collection.isPublished ? t('Published') : t('Draft')}</Badge></div></CardHeader><CardContent className="flex items-center justify-between"><span className="text-sm text-muted-foreground">{collection._count.articles} {t(collection._count.articles === 1 ? 'article' : 'articles')}</span><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => editCollection(collection)}><Pencil className="mr-1 h-3.5 w-3.5" />{t('Edit')}</Button><Button size="sm" variant="ghost" className="text-destructive" disabled={collection._count.articles > 0} onClick={() => window.confirm('Delete this empty collection?') && deleteCollection.mutate(collection.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div></CardContent></Card>
                    ))}
